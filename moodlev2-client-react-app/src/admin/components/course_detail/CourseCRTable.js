...
import RemoveCourseCR from "./RemoveCourseCR";
...
const CourseCRTable = ({ adminNo, courseId }) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				// fetch change hobe
				const response = await fetch(`/api/admin/course/${courseId}/crs`);
		...
		<>
			{showDelete && (
				<div style={{ alignSelf: "flex-start" }}>
					<RemoveCourseCR courseId={courseId} CRList={selectedCRsList} />
				</div>
			)}
