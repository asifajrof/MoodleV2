...
const rescheduleRequestDenied = async (req, res, next) => {
  try {
    const { sec, eventStartTime, eventEndTime, userName } = req.body;

    const start = moment(eventStartTime);
    const start__ = start.format("YYYY-MM-DD HH:mm:ssZZ");

    const end = moment(eventEndTime);
    const end__ = end.format("YYYY-MM-DD HH:mm:ssZZ");

    let result = await pool.query(
      "select json_agg(t) FROM add_extra_class($1,$2,$3,$4) as t",
      [sec, userName, start__, end__]
    );

    res.json({ message: "new extra class added", data: [] });
  } catch (err) {
    return next(new HttpError(err.message, 500));
  }
};
...
const getExtraClassRescheduleInfo = async (req, res, next) => {
  try {
    ...
    if (extra != null) {
      const obj = {
        userName: extra.crname,
        eventStartTime: moment(extra.start_time).format("LLL"),
        courseName: extra.dept_shortname + " " + extra.course_code,
        secName: extra.secname,
        secNo: extra.secno,
        eventEndTime: moment(extra.end_time).format("LLL"),
      };
...
const rescheduleRequestAccepted = async (req, res, next) => {
  try {
    ...
    let result = await pool.query(
      ...
      [eventId, givenTime, givenTimePlusOneHour]
    );

    res.json({ message: "rescheduleRequestAccepted", data: [] });
  } catch (err) {
    return next(new HttpError(err.message, 500));
  }
};

exports.getRescheduleEventsStudent = getRescheduleEventsStudent;
...
exports.rescheduleRequestDenied = rescheduleRequestDenied;
exports.rescheduleRequestAccepted = rescheduleRequestAccepted;
